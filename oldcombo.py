...
'''This is working i3d model, and a blip model and it conjoins the results, uses gpt '''
...
def blip_i3d_combo_story(descriptions):
    ...
    "Do not repeat the observations, summarize all of them into one summary. \n\n"
    ...
    with torch.no_grad():
        outputs = gpt_model.generate(inputs['input_ids'], 
                                    ...
                                    temperature=0.95,         # Controls creativity
                                    top_p=0.9,               # Nucleus sampling for more natural outputs
                                    do_sample=True, 
                                    max_new_tokens=200 )

    story = tokenizer.decode(outputs[0], skip_special_tokens=True)
    print(f"Combo Story of Blip and I3D:\n {story}")
    return story
...
def vid_preprocess(video_path, output_dir, seq_frames=16):
    ...
    while video.isOpened(): #keep open for all frames
        ...
        if len(i3d_frames) == seq_frames:
            ...
            #print(f"i3d shape used in predict actions idv: {i3d_video_tensor.shape}")
    ...
    print(f"Final Shape of i3d for batches: {i3d_video_tensor.shape}")
    return (i3d_video_tensor.float() / 255.0 - 0.5) / 0.5, blip_frames
...
print(f"Final Description from I3D ONLY: {description}")
